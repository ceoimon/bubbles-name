...
(function (root, factory) {
  ...
  let CONFIG = {
    radius: 150,
    shakeRaduis: 50,
    shakeRange: () => Math.floor(5 * Math.random()) - 2,
    friction: 0.85,
    rotationForce: 0,
    springStrength: 0.1,
    zRate: 100
  ...
  class Vector {
     ...
     */
    static calculate2DDistances (curPos, orgPos) {
      const distanceX = curPos.x - orgPos.x
      const distanceY = curPos.y - orgPos.y
      return {
        distanceX,
        distanceY,
  ...
  class PointCollection {
      ...
      this.points.forEach((point) => {
        ...
        */
        ...
        if (lineardistance < CONFIG.shakeRaduis) {
          this.pointCollectionX = CONFIG.shakeRange()
          this.pointCollectionY = CONFIG.shakeRange()
        }
        point.update()
      })
    }
     ...
     */
    draw (context, pointShape, reset = false) {
      this.points.forEach((point) => {
        if (reset) {
          this.pointCollectionY = this.pointCollectionX = 0
          this.mousePos = new Vector(0, 0)
        }
        point.draw(context, pointShape, this.pointCollectionX, this.pointCollectionY)
      })
    }
  }
  ...
  class Point {
    ...
    update () {
      const {
        distanceX,
        distanceY
      } = Vector.calculate2DDistances(this.targetPos, this.curPos)
      const {
        lineardistance
      } = Vector.calculate2DDistances(this.originalPos, this.curPos)
     ...
     */
    draw (context, pointShape, pointCollectionX, pointCollectionY) {
  ...
  class BubblesCanvas {
     ...
     */
    constructor (canvasElement, canvasWidth, canvasHeight, bubbleShape = 'circle') {
  ...
  function drawString ({
    ...
    const bubblesCanvas = new BubblesCanvas(element, width, height, shape)
    let stringHex = phraseToHex(string)
    let points = []
    let totalWidth = 0
    let _colorCounter = -1
    let colorCounter
    for (let strLengthCounter = 0; strLengthCounter < stringHex.length; strLengthCounter += 2) {
      let alphabetCode = 'A' + stringHex.charAt(strLengthCounter) + stringHex.charAt(strLengthCounter + 1)
      ...
      if (alphabet.hasOwnProperty(alphabetCode)) {
        const alphabetPoints = alphabet[alphabetCode].P
        colorCounter = colors[colorCounter % colors.length]
        for (let alphabetPCounter = 0; alphabetPCounter < alphabetPoints.length; ++alphabetPCounter) {
          let point = alphabetPoints[alphabetPCounter]
