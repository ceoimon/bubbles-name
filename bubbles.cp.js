 ...
 * Refactoring
 * Fork from http://s3.amazonaws.com/codecademy-content/courses/hour-of-code/js/bubbles.js
...
(function (root, factory) {
  ...
  var CONFIG = {
    radius: 150,
    shakeRaduis: 50,
    shakeRange: function shakeRange() {
  ...
  // Contain a-z and 0-9 characters' data for BubblesCanvas.
  var alphabet = {
    A79: {
      W: 75,
    ...
    A69: {
      W: 41,
      P: [[23, 60, 10, -92], [22, 91, 9, -36], [21, 111, 8, 39], [19, 130, 7, 56], [21, 148, 6, 112]]
    },
    A64: {
      W: 84,
    ...
    A5e: {
      W: 83,
      P: [[20, 70, 9, -79], [65, 68, 9, -79], [33, 58, 7, 50], [52, 58, 7, 50], [42, 48, 6, 112]]
    },
    A5d: {
      W: 55,
    ...
    A2c: {
      W: 41,
      P: [[30, 150, 10, -79], [24, 164, 7, 50], [17, 177, 6, 102]]
    },
    A2d: {
      W: 60,
      P: [[12, 118, 9, -84], [29, 117, 8, 69], [46, 117, 6, 5]]
    },
    A2e: {
      W: 37,
      P: [[19, 152, 6, -35]]
    ...
    A27: {
      W: 56,
      P: [[27, 72, 5, -35], [25, 62, 5, -35], [27, 51, 5, -35]]
    },
    A28: {
      W: 53,
    ...
    A3b: {
      W: 44,
      P: [[21, 91, 10, -79], [21, 144, 8, 42], [11, 155, 6, 102]]
    },
    A3a: {
      W: 44,
      P: [[21, 134, 10, -79], [23, 86, 10, 51]]
    },
    A3f: {
      W: 75,
  ...
  /** Class representing a three dimension vector. */
  ...
  var Vector = function () {
    ...
    _createClass(Vector, [{
      ...
      /**
       * Calculate distances between two 2D vector.
       * @param {Vector} currentPos - current position.
       * @param {Vector} originPos - original position.
       * @return {Object} Object contains x distance, y distance, and lineardistance
       */

    }], [{
      key: 'calculate2DDistances',
      value: function calculate2DDistances(curPos, orgPos) {
        var distanceX = curPos.x - orgPos.x;
        var distanceY = curPos.y - orgPos.y;
        return {
          distanceX: distanceX,
          distanceY: distanceY,
          lineardistance: Math.sqrt(distanceX * distanceX + distanceY * distanceY)
        };
      }
    }]);
  ...
  /** Class representing a point collection. */


  var PointCollection = function () {
    /**
     * Create a pointCollection.
     * @param {number} radius - distance for judge points update or not.
     */
    ...
    function PointCollection() {
      ...
      this.pointCollectionY = this.pointCollectionX = 0;
      this.points = [];
    }

    /**
     * Calculate the distance between point and mouse for every point in collection.
     * If distance less than radius, move the point.
     * Else reset the point position.
     */
    ...
    _createClass(PointCollection, [{
      ...
      value: function update() {
        ...
        this.points.forEach(function (point) {
          var _Vector$calculate2DDi = Vector.calculate2DDistances(_this.mousePos, point.curPos);

          var distanceX = _Vector$calculate2DDi.distanceX;
          ...
          */
          Math.random();
          if (lineardistance < CONFIG.shakeRaduis) {
            _this.pointCollectionX = CONFIG.shakeRange();
            _this.pointCollectionY = CONFIG.shakeRange();
          }
          point.update();
        });
      }
      ...
      /**
       * draw points.
       * @param {string} pointShape - point's shape.
       * @param {boolean=} reset - reset or not.
       */

    }, {
      key: 'draw',
      value: function draw(context, pointShape) {
        var _this2 = this;
        ...
        this.points.forEach(function (point) {
          if (reset) {
            _this2.pointCollectionY = _this2.pointCollectionX = 0;
            _this2.mousePos = new Vector(0, 0);
          }
          point.draw(context, pointShape, _this2.pointCollectionX, _this2.pointCollectionY);
        });
      }
    }]);
  ...
  /** Class representing a point. (or bubble?) */
  ...
  var Point = function () {
    ...
    function Point(x, y, z, size, color) {
      ...
      this.rotationForce = CONFIG.rotationForce;
      this.springStrength = CONFIG.springStrength;
      this.originalPos = new Vector(x, y, z); // Original position
      this.size = this.radius = size;
      this.targetPos = new Vector(x, y, z); // Target position
    ...
    _createClass(Point, [{
      key: 'update',
      value: function update() {
        var _Vector$calculate2DDi2 = Vector.calculate2DDistances(this.targetPos, this.curPos);

        var distanceX = _Vector$calculate2DDi2.distanceX;
        var distanceY = _Vector$calculate2DDi2.distanceY;

        var _Vector$calculate2DDi3 = Vector.calculate2DDistances(this.originalPos, this.curPos);

        var lineardistance = _Vector$calculate2DDi3.lineardistance;


        this.velocity.x = (this.velocity.x + (distanceX * this.springStrength - distanceY * this.rotationForce)) * this.friction;
        this.velocity.y = (this.velocity.y + (distanceY * this.springStrength - distanceX * this.rotationForce)) * this.friction;
        this.targetPos.z = lineardistance / CONFIG.zRate + 1;
        this.velocity.z = (this.velocity.z + (this.targetPos.z - this.curPos.z) * this.springStrength) * this.friction;
       ...
       * draw point.
       * @param {string} pointShape - point's shape.
       * @param {number} pointCollectionX - offset.
       * @param {number} pointCollectionY - offset.
       */

    }, {
      key: 'draw',
      value: function draw(context, pointShape, pointCollectionX, pointCollectionY) {
        context.fillStyle = this.color;
        if (pointShape === 'square') {
          context.beginPath();
          context.fillRect(this.curPos.x + pointCollectionX, this.curPos.y + pointCollectionY, 1.5 * this.radius, 1.5 * this.radius); // Maybe Rectangle is look visually bigger?
        } else {
            context.beginPath();
            context.arc(this.curPos.x + pointCollectionX, this.curPos.y + pointCollectionY, this.radius, 0, 2 * Math.PI, !0);
            context.fill();
          }
      }
    }]);
  ...
  /** Class representing a bubbles's canvas. */


  var BubblesCanvas = function () {
    /**
     * Create a BubblesCanvas.
     * @param {node} canvasElement - The canvas element.
     * @param {number} canvasWidth - The canvas width.
     * @param {number} canvasHeight - The canvas height.
     * @param {string=} bubbleShape - The bubble shape.
     */

    function BubblesCanvas(canvasElement, canvasWidth, canvasHeight) {
      var bubbleShape = arguments.length <= 3 || arguments[3] === undefined ? 'circle' : arguments[3];

      _classCallCheck(this, BubblesCanvas);
      ...
      // Bind the context for asynchronously call.
    ...
    /** Initialization, deal with events' binding then start the motion. */


    _createClass(BubblesCanvas, [{
      key: 'init',
      value: function init() {
      ...
      value: function getOffset(element) {
        ...
        if (element.offsetParent != null) {
          offsetLeft += this.getOffset(element.offsetParent).offsetLeft;
          offsetTop += this.getOffset(element.offsetParent).offsetTop;
        }
        return {
      ...
      value: function onMove(e) {
        var _getOffset = this.getOffset(this.canvas);
       ...
       * And get and set current mouse postion.
       * @param {Object} e - The event object.
       */
      ...
      value: function onTouchMove(e) {
        if (e.targetTouches && e.targetTouches.length === 1) {
          ...
          var _getOffset2 = this.getOffset(this.canvas);
      ...
      // No need to reset the canvas when mouse enter the window.
      ...
      // Drawing the bubbles constantly.

    }, {
      key: 'bounceBubbles',
      value: function bounceBubbles() {
        this.draw(this.bubbleShape);
        this.update();
      ...
      /**
       * Drawing the bubbles.
       * @param {string} bubbleShape - The bubbles' shape.
       */

    }, {
      key: 'draw',
      value: function draw(bubbleShape) {
        var context = this.canvas.getContext('2d');
        context.clearRect(0, 0, this.canvasWidth, this.canvasHeight);
        this.pointCollection.draw(context, bubbleShape, this.reset);
      }
    ...
    return BubblesCanvas;
  }();

  /**
   * Generate a hsl format color string.
   * @param {array} color - color array contain hue, saturation, lightness.
   * @return {string} A hsl format color string.
   */
  ...
  /**
   * Parse a string to hex format.
   * @param {array} string - original string.
   * @return {string} A hex format string.
   */
  function phraseToHex(str) {
    var hexStr = '';
    for (var counter = 0; counter < str.length; counter++) {
      hexStr += str.charCodeAt(counter).toString(16);
    }
    return hexStr;
  }

  var Colors = [[0, 100, 63], // red
  [40, 100, 60], // orange
  [75, 100, 40], // green
  [196, 77, 55], // blue
  [280, 50, 60] // purple
  ...
  /**
   * Drawing a string.
   * @param {Object} anonymous - augment object.
   * @param {node} anonymous.element - target canvas element.
   * @param {number} anonymous.width - canvas's width.
   * @param {number} anonymous.height - canvas's height.
   * @param {string} anonymous.string - string for draw.
   * @param {array} anonymous.colors - colors.
   */
  function drawString(_ref) {
    ...
    var bubblesCanvas = new BubblesCanvas(element, width, height, shape);
    var stringHex = phraseToHex(string);
    var points = [];
    var totalWidth = 0;
    var _colorCounter = -1;
    var colorCounter = void 0;
    for (var strLengthCounter = 0; strLengthCounter < stringHex.length; strLengthCounter += 2) {
      var alphabetCode = 'A' + stringHex.charAt(strLengthCounter) + stringHex.charAt(strLengthCounter + 1);
      alphabetCode !== 'A20' && _colorCounter++;
      colorCounter = _colorCounter;
      var _colors = colors;

      if (typeof _colors !== 'undefined') {
        Object.prototype.toString.call(_colors) === '[object Array]' && Object.prototype.toString.call(_colors[0]) === '[object Array]' && (colors = _colors);
        Object.prototype.toString.call(_colors) === '[object Array]' && typeof _colors[0] === 'number' && (colors = [_colors]);
      } else {
        colors = [[0, 0, 27]];
      }

      if (alphabet.hasOwnProperty(alphabetCode)) {
        var alphabetPoints = alphabet[alphabetCode].P;
        colorCounter = colors[colorCounter % colors.length];
        for (var alphabetPCounter = 0; alphabetPCounter < alphabetPoints.length; ++alphabetPCounter) {
          var point = alphabetPoints[alphabetPCounter];
          points.push(new Point(point[0] + totalWidth, point[1], 0, point[2], makeColor(colorCounter, point[3])));
        }
        totalWidth += alphabet[alphabetCode].W;
      }
    }

    for (var pointCounter = 0; pointCounter < points.length; pointCounter++) {
      points[pointCounter].curPos.x = width / 2 - totalWidth / 2 + points[pointCounter].curPos.x;
      points[pointCounter].curPos.y = height / 2 - 105 + points[pointCounter].curPos.y;
      points[pointCounter].originalPos.x = width / 2 - totalWidth / 2 + points[pointCounter].originalPos.x;
      points[pointCounter].originalPos.y = height / 2 - 105 + points[pointCounter].originalPos.y;
    }
    bubblesCanvas.pointCollection.points = points;
    bubblesCanvas.init();
  }
